...
//UserDefineDialog ScintillaEditView::_userDefineDlg; // Mattes: not in AP
...
void ScintillaEditView::init(HINSTANCE hInst, HWND hPere)
{
   ...
   _userDefineDlg.init(_hInst, _hParent, this); // Mattes
...
void ScintillaEditView::setUserLexer(const TCHAR *userLangName)
{
   int setKeywordsCounter = 0;
    ...
    UserLangContainer * userLangContainer = 0; // Mattes userLangName ? _pParameter->getULCFromName(userLangName) : _userDefineDlg._pCurrentUserLang;
...
BufferID ScintillaEditView::attachDefaultDoc()
{
   ...
   BufferID id = 0; // Mattes    MainFileManager->bufferFromDocument(doc, false, true);//true, true);	//keep counter on 1
#if 0 // Mattes
...
void ScintillaEditView::restyleBuffer() {
  ...
  // _currentBuffer->setNeedsLexing(false);  // Mattes
}

void ScintillaEditView::styleChange() {
  // defineDocType(_currentBuffer->getLangType());  // Mattes
   restyleBuffer();
}
...
void ScintillaEditView::hideLines() {
   ...
   // _currentBuffer->setHideLineChanged(true, startLine-1); // Mattes
}
...
bool ScintillaEditView::markerMarginClick(int lineNumber) {
   ...
   if (openPresent) {
      _currentBuffer->setHideLineChanged(false, lineNumber); // Mattes: was commented
   }
   if (closePresent) {
      openPresent = false;
...
void ScintillaEditView::notifyMarkers(Buffer * buf, bool isHide, int location, bool del) {
   if (buf != _currentBuffer)	//if not visible buffer dont do a thing  // Mattes: was commented
      return;                                                            // Mattes: was commented
