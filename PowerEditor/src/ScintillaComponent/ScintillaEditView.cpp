...
// Mattes: not in AP: bool ScintillaEditView::_SciInit = false;
...
LRESULT ScintillaEditView::scintillaNew_Proc(HWND hwnd, UINT Message, WPARAM wParam, LPARAM lParam)
{
	switch (Message)
	{
		...
		case WM_RBUTTONDOWN:
		{
			bool rightClickKeepsSelection = false; // Mattes ((NppParameters::getInstance()).getSVP())._rightClickKeepsSelection;
...
void ScintillaEditView::setUserLexer(const TCHAR * /*userLangName*/) // Mattes
{
    ...
    UserLangContainer * userLangContainer = 0; // Mattes userLangName? NppParameters::getInstance().getULCFromName(userLangName):_userDefineDlg._pCurrentUserLang;
...
{
	const TCHAR* doxygenKeyWords = 0; // Mattes NppParameters::getInstance().getWordList(L_CPP, LANG_INDEX_TYPE2);
...
void ScintillaEditView::bufferUpdated(Buffer * buffer, int mask)
{
	...
	if (buffer == _currentBuffer)
	{
		...
		{
			// Mattes defineDocType(buffer->getLangType());
...
void ScintillaEditView::showAutoComletion(size_t lenEntered, const TCHAR* list)
{
	...
	// TODO Mattes NppDarkMode::setDarkAutoCompletion();
}
...
void ScintillaEditView::performGlobalStyles()
{
	...
	// Mattes ::SendMessage(_hParent, NPPM_INTERNAL_EDGEMULTISETSIZE, 0, 0);
...
void ScintillaEditView::setBorderEdge(bool doWithBorderEdge)
{
	...
	if (false) // Mattes TODO NppDarkMode::isEnabled())
	{
